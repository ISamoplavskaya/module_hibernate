    ...
    public List<T> findAll() {
        ...
        try (Session session = HibernateConnectionUtil.getSession()) {
            ...
            try {
                ...
                throw new HibernateSessionException("(Error searching for all entities during a Hibernate session", e);
            }
        }
        return allEntity;
    }
        ...
        if (Validator.validateEntity(entity)) {
            try (Session session = HibernateConnectionUtil.getSession()) {
                ...
                try {
                    ...
                    session.getTransaction().rollback();
                    throw new HibernateSessionException("(Error saving entity during a Hibernate session", e);
                }
            }
        ...
        if (Validator.validateEntity(entity)) {
            try (Session session = HibernateConnectionUtil.getSession()) {
                ...
                try {
                    ...
                    session.getTransaction().rollback();
                    throw new HibernateSessionException("(Error updating entity Hibernate session", e);
                }
            }
    ...
    public void delete(T entity) {
        try (Session session = HibernateConnectionUtil.getSession()) {
            ...
            try {
                ...
                session.getTransaction().rollback();
                throw new HibernateSessionException("(Error deleting entity Hibernate session", e);
            }
        }
    }
}
